...
-- 11b. CRITICAL: Compare seeds user_id vs users table
-- If these don't match, seeds won't be found!
SELECT 'SEEDS user_ids' as source, user_id, COUNT(*) as count
FROM seeds
GROUP BY user_id
UNION ALL
SELECT 'USERS in system' as source, id as user_id, 1 as count
FROM users
ORDER BY source, user_id;

-- 11c. MOST LIKELY ISSUE: Check if seed user_ids exist in users table
-- Run this to see if there's a user_id mismatch!
SELECT
    s.user_id as seeds_user_id,
    u.id as users_table_id,
    u.email,
    COUNT(*) as seed_count,
    CASE
        WHEN u.id IS NULL THEN '❌ USER NOT FOUND - seeds orphaned!'
        ELSE '✓ Match'
    END as status
FROM seeds s
LEFT JOIN users u ON s.user_id = u.id
GROUP BY s.user_id, u.id, u.email;

-- 11d. Check agent_jobs to see what user_id autonomous loop uses
SELECT
    id,
    user_id,
    job_type,
    ...
    started_at
FROM agent_jobs
WHERE job_type IN ('morning_briefing', 'evening_synthesis')
