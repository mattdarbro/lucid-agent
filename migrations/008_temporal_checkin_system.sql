...
-- Note: Run this manually in development only, not in production

/*
-- Create sample check-in preferences for first user
INSERT INTO check_in_preferences (user_id, enabled, morning_enabled, evening_enabled)
SELECT id, true, true, true FROM users LIMIT 1
...
-- This migration creates the infrastructure for temporal check-ins:
-- 1. Thought notifications - What Lucid wants to ask users
-- 2. User preferences - When users want to be contacted
-- 3. Multi-day research tasks - Long-running collaborative thinking
-- 4. State observations - Research data on cognitive states
-- 5. Schedule log - Tracking for rate limiting

-- To apply: psql -U your_user -d your_db -f migrations/008_temporal_checkin_system.sql
-- To rollback: DROP TABLE check_in_schedule_log, temporal_state_observations, multi_day_research_tasks, check_in_preferences, thought_notifications CASCADE;
