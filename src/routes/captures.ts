...
// ============================================================================
// POST /v1/captures/import-reminders - Bulk import from iOS Reminders
// ============================================================================
router.post('/import-reminders', async (req: Request, res: Response) => {
  try {
    const { user_id, reminders } = req.body;

    if (!user_id || !Array.isArray(reminders)) {
      return res.status(400).json({ error: 'user_id and reminders array are required' });
    }
    ...
    for (const reminder of reminders) {
      // Check if already imported (by external_id in source_metadata)
      const existingCheck = await pool.query(
        `SELECT id FROM captures
         WHERE user_id = $1
           AND source = 'ios_import'
           AND source_metadata->>'external_id' = $2`,
        [user_id, reminder.external_id]
      );

      if (existingCheck.rows.length > 0) {
        skipped.push({ external_id: reminder.external_id, reason: 'already_imported' });
        continue;
      }

      // Import the reminder as a capture
      const result = await pool.query(
        `INSERT INTO captures (
          user_id, content, source, source_metadata, status,
          has_deadline, deadline_at, priority, is_completed, completed_at
        )
        VALUES ($1, $2, 'ios_import', $3, 'inbox', $4, $5, $6, $7, $8)
        RETURNING *`,
        [
          user_id,
          reminder.title,
    ...
    logger.info(`Imported ${imported.length} reminders for user ${user_id}, skipped ${skipped.length}`);
    ...
    logger.error('Error importing reminders:', error);
